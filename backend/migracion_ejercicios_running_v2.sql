...
-- =====================================================
-- Fecha: 2026-03-09
-- Versión: 2.0 (Compatible con MySQL 8.0)
...
ALTER TABLE `ejercicio`
ADD COLUMN `distancia` VARCHAR(50) DEFAULT NULL AFTER `unidad`;

-- Agregar columna 'duracion' (para ejercicios basados en tiempo)
ALTER TABLE `ejercicio`
ADD COLUMN `duracion` VARCHAR(50) DEFAULT NULL AFTER `distancia`;

-- Agregar columna 'descripcionIntervalo' (ej: "3' suaves x 3' fuertes")
ALTER TABLE `ejercicio`
ADD COLUMN `descripcionIntervalo` TEXT DEFAULT NULL AFTER `duracion`;
...
ADD COLUMN `pausaSeries` VARCHAR(50) DEFAULT NULL AFTER `cantidad`;

-- Agregar columna 'intensidad' (rápidos, fondo, subida, etc.)
ALTER TABLE `rutina_ejercicio`
ADD COLUMN `intensidad` TEXT DEFAULT NULL AFTER `pausaSeries`;

-- Agregar columna 'calentamiento' (si es ejercicio de calentamiento)
ALTER TABLE `rutina_ejercicio`
ADD COLUMN `esCalentamiento` BOOLEAN DEFAULT FALSE AFTER `intensidad`;
...
ADD COLUMN `pausaSeries` VARCHAR(50) DEFAULT NULL AFTER `especificaciones`;
